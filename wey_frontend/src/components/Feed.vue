                      ...
                      <p>{{ post.body }}</p>
                        <div class="my-6 flex justify-between">
                            <div class="flex space-x-6">
                                <div class="flex items-center space-x-2" v-on:click="postLike(post.id)">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <RouterLink v-bind:to="{name:'postview', params: {id: post.id}}"> <span class="text-gray-500 text-xs">{{ post.comments_count }} comments</span> </RouterLink>
                                </div>
                            </div>
    ...
    methods: {
        postLike(id) {
            axios
                ...
                .then(response => {
                if (response.data.message == "like append" || response.data.message == "like up") {
                    this.post.likes_count += 1;
                    console.log("up");
                }
                else if (response.data.message == "like delete") {
                    this.post.likes_count -= 1;
                }
